...
int main(){
    ...
    ds = InsertToHead(ds,1);ds = InsertToHead(ds,2);
		ds = InsertToHead(ds,3);
    ds = InsertToHead(ds,4);ds = InsertToHead(ds,5);
...
/*
	// Xoa mot o du lieu
	printf("Cho vi tri ban muon xoa sau p = ");scanf("%d",&p);
    pv = ds;i=1;
    while(i<p){pv = pv->Next;i++;}
    Delete(pv);
    Print(ds);
*/
	// Chen o du lieu 7 vao dau danh sach
